...
%                 y_anchor = zeros(1, nearest_neighbor);
            ...
            for u=1:class_num
                ...
                anchors(anchor_idx,:) = anchors(anchor_idx,:) - learning_rate_anchor  / (idx + t0) * ((err(u)-1)*y(u)* base/(sum(weight).^2));
...
%%
plot(rmse_dadk_llfm_train(1,:),'r--x', 'DisplayName','LLFM-JO');
legend('-DynamicLegend');
hold on;
