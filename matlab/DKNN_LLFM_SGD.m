...
mse_dkllfm_sgd = zeros(1,iter_num*num_sample);
loss = zeros(1,iter_num*num_sample);

rmse_dkllfm_test = zeros(1, iter_num);
...
for i=1:iter_num
    ...
    for j=1:num_sample
        if mod(j,1000)==0  
            ...
            fprintf('average value of K in KNN is:%.2f\n', num_knn/((i-1)*num_sample + j));
        end
        ...
        % pick anchor points
        [anchor_idx, weight] = Dynamic_KNN(anchors, X);
        nearest_neighbor = length(anchor_idx);
        num_knn = num_knn + nearest_neighbor;
        
%         fprintf('value of K in KNN: %d\n', nearest_neighbor);
...
%         end
        ...
        loss(idx) = err^2;
        mse_dkllfm_sgd(idx) = sum(loss)/idx;
...
%         V = V - learning_rate * (2*err*(repmat(X',1,factors_num).*(repmat(X*temp_V,p,1)-repmat(X',[nearest_neighbor, 1,factors_num]).*V)) + 2*reg_v*V);
    ...
    rmse_dkllfm_test(i) = (mse_llfm_test / num_sample_test).^0.5;
end
