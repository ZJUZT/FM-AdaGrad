...
for i=1:iter_num
    ...
    mse_da_llfm_sgd = zeros(1,num_sample*epoch);
    loss = zeros(1,num_sample*epoch);
...
%             idx = (t-1)*num_sample + j;
            idx = j;
            if idx==1
                mse_da_llfm_sgd(idx) = -log(err_c);
                % mse_da_llfm_sgd(idx) = err^2;
            else
                % mse_da_llfm_sgd(idx) = (mse_da_llfm_sgd(idx-1) * (idx - 1) + err^2)/idx;
                mse_da_llfm_sgd(idx) = (mse_da_llfm_sgd(idx-1) * (idx - 1) -log(err_c))/idx;
            end
...
%%
% plot
plot(mse_da_llfm_sgd,'DisplayName','DALLFM\_Train');
