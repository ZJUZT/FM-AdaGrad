...
\textbf{Locally Linear FM(LLFM)}:FM with fixed anchor points which initialized by K-means.\\
\textbf{LLFM with Adaptive Anchor Points(LLFMAAP)}:Update anchor points during learning phase. The anchor points are also initialized by K-means.\\ 
\textbf{LLFMAAP with Adaptive KNN(LLFMAAPK)}:Choose the optimal number of nearest neighbors of KNN for each sample. Also using adaptive anchor points like the former algorithm.
\section{Conclusion}
